/**
 * PlantData.ts — Plant growth stages and harvest data matching Lua PlantData.lua.
 */
...
export interface AgeStage {
  ...
  spriteName: string;
  bCanBeEaten?: boolean;
  bCanBeHarvested?: boolean;
}

export interface HarvestInfo {
  tNumHarvestedRange: [number, number];
}
...
export interface PlantDef {
  ...
  nLifeTime: number;
  sPlantLC: string;
  tHarvestableFoods: Record<string, HarvestInfo>;
}

export const tPlants: Record<string, PlantDef> = {
  Corn: {
    ageInfo: [
  ...
  Glowfruit: {
    ...
    tHarvestableFoods: {
      Glowfruit: { tNumHarvestedRange: [3, 9] },
    },
  },
  CandyCane: {
    ageInfo: [
    ...
    tHarvestableFoods: {
      CandyCane: { tNumHarvestedRange: [1, 5] },
    },
  },
};
