...
| 2026-03-08 | Comprehensive Lua parity sweep (Ralph loop). Start menu: Load Base always visible, button order matches Lua (Resume/New/Tutorial/Load/Save/Settings/Credits). Gradient bars 180→64px (Lua grad64 texture). Cutaway mode: GameRules.cycleCutawayMode/enableCutawayMode, TileRenderer3D.setCutaway hides wall tops, construct menu saves/restores cutaway on open/close (Lua NewSideBar.lua). O2/Wall toggle buttons show persistent selected state (Lua StatusBar.onTick setSelected). NewGameScreen: canvas rotates 15°→0° during inspector slide-in (Lua DFMath.lerp). Save/load: persist cutawayMode, bDisasterMode, hint flags. Flip button UI for object placement (Lua StatusBar.rFlipButton). Speed buttons 32→40px (Lua scale 1.4). Disaster sidebar button (hidden until bDisasterMode). Alert auto-dismiss by nEndTime, deduplication. Inspector header: job title separate line, amber stats background. 240 tests. |
